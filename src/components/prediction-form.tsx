  ...
  return (
    <form onSubmit={submit} className="space-y-5">
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-2"><Label htmlFor="home">{homeTeam}</Label><Input id="home" type="number" min="0" step="1" value={home} onChange={(event) => setHome(event.target.value)} required /></div>
        <div className="space-y-2"><Label htmlFor="away">{awayTeam}</Label><Input id="away" type="number" min="0" step="1" value={away} onChange={(event) => setAway(event.target.value)} required /></div>
      </div>
